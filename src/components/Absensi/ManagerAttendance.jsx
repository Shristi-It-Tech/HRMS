// src/components/Attendance/ManagerAttendance.jsx
// Mirrors EmployeeAttendance so managers share the same attendance experience.
...
const ManagerAttendance = ({ user, employees, setEmployees }) => {
        ...
        <div>
            ...
            <GlassCard>
                <h3 className="text-xl font-bold mb-4 text-gray-700 border-b pb-2">
                    Attendance History ({new Date().toLocaleString('en-US', { month: 'long' })})
                </h3>
                        ...
                        <div key={index} className={`p-3 rounded-lg shadow-sm flex justify-between items-center ${att.type === 'Clock In' ? (att.late ? 'bg-red-50' : 'bg-green-50') : 'bg-gray-50'}`}>
                            <div className="flex-1">
                                <p className="font-semibold text-gray-800">
                                    {att.date}
                                    <span className={`text-xs ml-2 px-2 py-0.5 rounded-full ${att.type === 'Clock In' ? 'bg-blue-200 text-blue-800' : 'bg-orange-200 text-orange-800'}`}>
                                        {att.type}
                                    </span>
                                </p>
